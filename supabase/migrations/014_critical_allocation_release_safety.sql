...
--
-- This migration closes two high-impact correctness gaps:
--   1. Weekly allocation saves must be atomic and idempotent by week.
--   2. Fixed-spot release markers must be day-specific and maintained inside
--      the release/reclaim RPCs.
...
REVOKE ALL ON FUNCTION public.release_and_promote(UUID, INTEGER, DATE) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.release_fixed_and_promote(UUID, INTEGER, DATE) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.reclaim_fixed_spot(UUID, INTEGER, DATE) FROM PUBLIC;
