...
export async function GET(request: NextRequest) {
  try {
    ...
    // ── DST-safe cron guard ──────────────────────────────────────────────
    // GitHub Actions cron runs in UTC. The workflow schedules both
    // 18:00 UTC and 19:00 UTC each Wednesday — exactly one starts at
    // 19:00 Lisbon depending on DST. Admin manual triggers bypass the
    // guard.
    if (isCronAuthed) {
      const nowLisbon  = toZonedTime(new Date(), LISBON_TIMEZONE)
      const lisbonHour = nowLisbon.getHours()
      // Keep this exact. With the DST-safe UTC pair, a wider window would
      // let both scheduled runs send reminder emails during summer time.
      if (
