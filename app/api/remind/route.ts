...
export async function GET(request: NextRequest) {
  try {
    ...
    // ── DST-safe cron guard ──────────────────────────────────────────────
    // GitHub Actions cron runs in UTC.  We schedule both 18:00 UTC and
    // 19:00 UTC each Wednesday — exactly one matches 19:00 Lisbon
    // depending on DST. Skip the run if Lisbon time isn't the target
    // (Wed 19:00). Admin manual triggers bypass the guard.
    if (isCronAuthed) {
