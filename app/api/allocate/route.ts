...
async function handleAllocate(request: NextRequest, weekStartOverride?: string) {
  ...
  if (isCronAuthed) {
    ...
    // Hour guard must be exact: registration stays open until 08:00
    // Lisbon, so accepting earlier cron fires can publish allocations
    // before all valid requests have been submitted.
    if (
