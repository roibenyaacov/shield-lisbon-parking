...
async function handleAllocate(request: NextRequest, weekStartOverride?: string) {
  ...
  if (isCronAuthed) {
    ...
    // Day/hour guard: registration remains open until Friday 08:00.
    // The paired UTC schedules cover both Lisbon DST offsets; only the
    // schedule that lands in the target local hour may publish results.
    if (
