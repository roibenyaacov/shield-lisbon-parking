...
async function handleAllocate(request: NextRequest, weekStartOverride?: string) {
  ...
  // ── DST-safe cron guard ────────────────────────────────────────────────
  // GitHub Actions schedules are UTC-only, so the workflow calls this
  // endpoint at both adjacent UTC hours. Exactly one lands in the target
  // Lisbon hour depending on DST.
  if (isCronAuthed) {
